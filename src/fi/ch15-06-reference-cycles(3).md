## Viittaussilmukat voivat aiheuttaa muistivuotoja

Rustin muistiturvallisuustakuu tekee vaikeaksi, mutta ei mahdottomaksi
luoda vahingossa muistia, jota ei koskaan vapauteta (tunnetaan _muistivuotona_).
Muistivuotojen täydellinen estäminen ei kuulu Rustin takuihin, meaning
memory leaks are memory safe in Rust. Voimme nähdä, että Rust sallii muistivuodot
käyttämällä `Rc<T>` ja `RefCell<T>`: it’s possible to create references where
items refer to each other in a cycle. This creates memory leaks because the
reference count of each item in the cycle will never reach 0, and the values
will never be dropped.

### Viittaussilmukan luominen

Tarkastellaan, miten viittaussilmukka voi syntyä ja miten sitä voi estää,
...
Käytämme toista versiota `List`-määritelmästä kohdasta 15-5. The
second element in the `Cons` variant is now `RefCell<Rc<List>>`, meaning that
instead of having the ability to modify the `i32` value as we did in Listing
15-24, haluamme muokata `List`-arvoa, johon `Cons`-vaihtoehto viittaa.
